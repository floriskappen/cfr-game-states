...
mod poker_tests_multiplayer {
    ...
    fn test_turn_order_simple() {
        ...
        game_state = game_state.handle_action(Action::Call); // Player 3 calls
        assert_eq!(game_state.active_player_index, 3);
        game_state = game_state.handle_action(Action::Call); // Player 4 calls
        assert_eq!(game_state.active_player_index, 4);
        game_state = game_state.handle_action(Action::Call); // Player 5 calls
        assert_eq!(game_state.active_player_index, 5);
        game_state = game_state.handle_action(Action::Call); // Player 6 calls
    ...
    fn test_turn_order_complex_betting() {
        ...
        game_state = game_state.handle_action(Action::Call); // Player 6 calls
        assert_eq!(game_state.active_player_index, 0);
        game_state = game_state.handle_action(Action::Bet1); // Player 1 bets 1x pot
        assert_eq!(game_state.active_player_index, 1);
        game_state = game_state.handle_action(Action::Fold); // Player 2 calls
        assert_eq!(game_state.active_player_index, 2);
        game_state = game_state.handle_action(Action::Call); // Player 3 calls
        assert_eq!(game_state.active_player_index, 3);
        game_state = game_state.handle_action(Action::Bet0_5); // Player 4 bets 0.5x pot
        assert_eq!(game_state.active_player_index, 4);
        game_state = game_state.handle_action(Action::Call); // Player 5 calls
        assert_eq!(game_state.active_player_index, 5);
        game_state = game_state.handle_action(Action::Fold); // Player 6 folds
        assert_eq!(game_state.active_player_index, 0);
        game_state = game_state.handle_action(Action::Call); // Player 1 calls
        assert_eq!(game_state.active_player_index, 2);
        assert_eq!(game_state.round, 0);
        game_state = game_state.handle_action(Action::Call); // Player 3 calls (skip player 2 cuz he folded)
        assert_eq!(game_state.round, 1);
        assert_eq!(game_state.active_player_index, 0);
        assert_eq!(game_state.is_terminal(), false);
    }
    ...
    fn test_complex_checking_1() {
        ...
        game_state = game_state.handle_action(Action::Call); // Player 4 calls
        assert_eq!(game_state.active_player_index, 4);
        game_state = game_state.handle_action(Action::Fold); // Player 5 calls
        assert_eq!(game_state.active_player_index, 5);
        game_state = game_state.handle_action(Action::Call); // Player 6 calls
        assert_eq!(game_state.active_player_index, 0);
        game_state = game_state.handle_action(Action::Call); // Player 1 bets 1x pot
        assert_eq!(game_state.active_player_index, 1);
        assert_eq!(game_state.round, 0);
        game_state = game_state.handle_action(Action::Call); // Player 2 calls
        assert_eq!(game_state.round, 1);
    ...
    fn test_complex_checking_2() {
        ...
        // Simulate several players folding
        assert_eq!(game_state.active_player_index, 2);
        game_state = game_state.handle_action(Action::Bet1_5); // Player 3 bets
        assert_eq!(game_state.active_player_index, 3);
        game_state = game_state.handle_action(Action::Fold); // Player 4 folds
        assert_eq!(game_state.active_player_index, 4);
        game_state = game_state.handle_action(Action::Bet1); // Player 5 bets
        assert_eq!(game_state.active_player_index, 5);
        game_state = game_state.handle_action(Action::Call); // Player 6 calls
        assert_eq!(game_state.active_player_index, 0);
        game_state = game_state.handle_action(Action::Call); // Player 1 bets 1x pot
    ...
    fn test_complex_checking_3() {
        ...
        game_state = game_state.handle_action(Action::Call); // Player 4 calls
        assert_eq!(game_state.active_player_index, 4);
        game_state = game_state.handle_action(Action::Bet1_5); // Player 5 bets
        assert_eq!(game_state.active_player_index, 5);
        game_state = game_state.handle_action(Action::Call); // Player 6 calls
        assert_eq!(game_state.active_player_index, 0);
        game_state = game_state.handle_action(Action::Fold); // Player 1 folds
        assert_eq!(game_state.active_player_index, 1);
        game_state = game_state.handle_action(Action::Fold); // Player 2 folds
        assert_eq!(game_state.active_player_index, 2);
        game_state = game_state.handle_action(Action::Fold); // Player 3 folds
        assert_eq!(game_state.active_player_index, 3);
        assert_eq!(game_state.round, 0);
        assert_eq!(game_state.all_remaining_players_checked(), false);
        game_state = game_state.handle_action(Action::Call); // Player 4 calls
    ...
    fn test_initial_folds() {
        let mut game_state = setup_game_state_six_players();
        // Simulate several players folding
        game_state = game_state.handle_action(Action::Fold); // Player 0 folds
        game_state = game_state.handle_action(Action::Fold); // Player 1 folds
        game_state = game_state.handle_action(Action::Fold); // Player 2 folds
        game_state = game_state.handle_action(Action::Fold); // Player 3 folds
